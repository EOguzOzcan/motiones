...
if(mockdata[0]){
	return(
		<Tabs  color="orange" className="mb-16 mt-12">
		<>
			<Tabs.List>
	{mockdata[0].data.map((item) => (
...
{mockdata[1].data.map((item) => (
	...
	const features = mockdata.map((feature) => (
		...
		<RenderData/>
			...
			<div className="absolute bottom-0 right-0 mr-8 ">
				<Button
					fullWidth
					className="bg-gray-500 mb-4 w-32 hover:bg-orange-500 transition-all"
	...
	return (
		<div className="px-12 py-6">
			<Title
				className={`${classes.title} text-2xl md:text-4xl mt-2 mb-6 text-black text-center`}
				order={2}
			>
				Products
			</Title>
			<Container size={800} p={0}>
				<Text
					size="sm"
					className={`${classes.description} text-lg  text-center mb-12`}
				>
					At ARC Global Chain, we take pride in offering a diverse range of
					premium sugar and top-notch sunflower oil products
				</Text>
			</Container>
			<SimpleGrid
				cols={2}
